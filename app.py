...
mydb=mysql.connector.Connect(host='localhost',user='root',password='admin',db='bidzone')
Session(app)
...
def biditems(item_id):
    ...
    if session.get('user'):
        ...
        if winning_bid_amount is not None and winning_bid_amount[0] is not None:
...
def placed_bids(itemid):
    if session.get('user'):
        if request.method == "POST":
            ...
            item_data = cursor.fetchone()
            starting_price = item_data[0]
            #print('===================', starting_price)

            if bid_amount >= 0 and bid_amount > starting_price:
                flash('Bid amount must be below the item starting price and not below zero')
                return redirect(url_for('userviewitems'))

            # Checking if the particular itemid is found in auction data; if not found, insert
            cursor.execute('SELECT count(*) FROM auction_data WHERE item_id=%s', [itemid])
            count = cursor.fetchone()[0]
            print(count)
            if count == 1:
                # Fetch the current lowest bid
                cursor.execute('SELECT auction_id FROM auction_data WHERE item_id=%s', [itemid])
                auction_id = cursor.fetchone()
                cursor.execute('SELECT bid_amount FROM placed_bids WHERE item_id = %s ORDER BY bid_amount ASC LIMIT 1',
                               [itemid])
                current_bid_data = cursor.fetchone()
                cursor.execute('SELECT starting_price FROM items WHERE item_id=%s', [itemid])
                ...
                current_bid = current_bid_data[0] if current_bid_data else None  # Set current bid to None if no bids yet
                bid_decrement = current_bid - bid_amount if current_bid is not None else starting_price - bid_amount  # Calculate bid decrement

                if bid_amount < 0 and bid_decrement < 0:
                    flash('Invalid bid amount or bid decrement')
                    return redirect(url_for('userviewitems'))
                # if current_bid:
                # # Check if the new bid amount is higher than the previous lowest bid amount
                #     if bid_amount >= current_bid:
                #         flash('Bid amount must be lower than the previous highest bid amount')
                #         return redirect(url_for('userviewitems'))

                # Store the bid details in placed_bids table
                cursor = mydb.cursor(buffered=True)
                cursor.execute('INSERT INTO placed_bids (auction_id, item_id, bidder_username, bid_amount, bid_time, current_bid, bid_decrement) VALUES (%s, %s, %s, %s, %s, %s, %s)',
                    (auction_id[0], itemid, bidder_username, bid_amount, bid_time, current_bid, bid_decrement))
                mydb.commit()
                cursor.close()
                ...
                cursor.execute('SELECT starting_price FROM items WHERE item_id=%s', [itemid])
                starting_price = cursor.fetchone()[0]
                cursor.execute('INSERT INTO auction_data (item_id, seller_username, starting_price, auction_start, status) VALUES (%s, %s, %s, %s, %s)',
                    (itemid, seller_username, starting_price, bid_time, 'Active'))
                mydb.commit()
                ...
                auction_id = cursor.fetchone()
                cursor.execute(
                    'SELECT bid_amount FROM placed_bids WHERE item_id = %s ORDER BY bid_amount ASC LIMIT 1',
                    [itemid])
                current_bid_data = cursor.fetchone()
                if current_bid_data:
                    current_bid = current_bid_data[0] if current_bid_data else None  # Set current bid to None if no bids yet
                    ...
                    if bid_amount < 0 or bid_decrement < 0:
                        flash('Invalid bid amount or bid decrement')
                        return redirect(url_for('userviewitems'))
                current_bid = current_bid_data[0] if current_bid_data else None  # Set current bid to None if no bids yet
                bid_decrement = current_bid - bid_amount if current_bid is not None else starting_price - bid_amount  # Calculate bid decrement
...
def stop_auction(item_id):
    if session.get('seller'):
        ...
        stop_auction = datetime.datetime.now()
        cursor = mydb.cursor(buffered=True)
        cursor.execute('UPDATE auction_data SET status = "Completed",auction_end = %s  WHERE item_id = %s', [stop_auction,item_id])
        cursor.execute('UPDATE items SET status = "Completed" WHERE item_id = %s', [item_id])
...
def userbiditems():
    if session.get('user'):
        ...
        cursor = mydb.cursor(buffered=True)
        cursor.execute('''
            SELECT pb.bid_amount,bidder_username,bid_decrement, pb.bid_time, pb.current_bid, i.item_name,i.seller_username,i.dis,i.starting_price,i.img_id
            FROM placed_bids pb
            JOIN items i ON pb.item_id = i.item_id
            WHERE pb.bidder_username = %s
        ''', (session['user'],))
        user_bids = cursor.fetchall()
        print('===========================================',user_bids)
        
        return render_template('userbiditems.html',user_bids=user_bids)
    #return render_template('userbiditems.html')


    else:
        return redirect(url_for('ulogin'))
@app.route('/wiining_bid_details')
def wiining_bid_details():
    if session.get('user'):
        cursor = mydb.cursor(buffered=True)
        cursor.execute("""
            SELECT ad.winning_bid_amount, ad.auction_end, ad.seller_username, i.item_name,i.seller_username,i.dis,i.starting_price,i.img_id
            FROM auction_data ad
            JOIN items i ON ad.item_id = i.item_id
            WHERE ad.winning_bidder_username = %s and ad.status=%s
        """, (session['user'],'Completed'))
        winning_auctions = cursor.fetchall()

        return render_template('auction_winning.html',winning_auctions=winning_auctions)

    return redirect(url_for('ulogin'))
#==========================user payements
'''
cursor.execute('SELECT * FROM auction_data WHERE winning_bidder_username = %s and status=%s', [session['user'],'Completed'])
        items= cursor.fetchall()
...
def pay(itemid):
    if session.get('user'):
        cursor = mydb.cursor(buffered=True)
        cursor.execute('SELECT * FROM orders WHERE user_name = %s AND  item_id= %s', (session['user'], itemid))
        already_paid = cursor.fetchone()

        if already_paid:
            flash('You have already purchased.')
            return redirect(url_for('userviewitems')) 
...
def success(itemid,name,total):
    if session.get('user'):
        ...
        cursor.execute('SELECT winning_bid_amount from auction_data where winning_bidder_username=%s',[session['user']])
        winning_bid_amount=cursor.fetchone()[0]
...
def sforgot():
    if request.method=='POST':
        ...
        if count==1:
            ...
            cursor.execute('SELECT email from sellers where seller_username=%s',[id1])
            email=cursor.fetchone()[0]
            cursor.close()
            subject='Forget Password'
...
def sreset(token):
    try:
        ...
        if request.method=='POST':
            ...
            if newpassword==confirmpassword:
                cursor=mydb.cursor(buffered=True)
                cursor.execute('update sellers set password=%s where seller_username=%s',[newpassword,id1])
                mydb.commit()
                flash('Reset Successful')
                return redirect(url_for('slogin'))
            else:
...
def additems():
    if session.get('seller'):
        ...
        if request.method=="POST":
            ...
            mydb.commit()
            cursor.execute('select email from the users')
            emails = cursor.fetchall()
            if emails:
                for i in emails:
                    subject='BID ZONE'
                    body=f"Hey user {itemname} newly added for biding you can bid on it "
...
def additems():
    if session.get('seller'):
        ...
        if request.method == "POST":
            ...
            filename = id1_pic + '.jpg'
            cursor = mydb.cursor(buffered=True)
            cursor.execute('INSERT INTO items (seller_username, item_name, dis, category, starting_price, img_id) VALUES (%s, %s, %s, %s, %s, %s)',
                           [session['seller'], itemname, dis, category, starting_price, id1_pic])
            mydb.commit()
            cursor.close()
            path = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'static')
            if not os.path.exists(path):
            ...
            if emails:
                subject = 'New Item for Bidding'
                body = f"Hello,\n\nA new item '{itemname}' has been added for bidding on our platform.\n\nHappy Bidding!"
                for email in emails:
                    sendmail(to=email,subject=subject,body=body)
                    
                

            flash('Item added successfully, and notifications sent to registered users.')
            return redirect(url_for('seller_dashboard'))
        return render_template('additems.html')
    else:
        return redirect(url_for('slogin'))
...
def deleteitem(itemid):
    if session.get('seller'):
        ...
        cursor=mydb.cursor(buffered=True)
        cursor.execute('SELECT img_id FROM items WHERE item_id=%s',[itemid])
        
        img_id = cursor.fetchone()[0]
        
        cursor.execute('SELECT item_id FROM auction_data WHERE item_id=%s',[itemid])
        img_id1 = cursor.fetchone()
        if img_id1:
            flash('you can not delete the item because this item is under the bidding')
            return redirect(url_for('seller_dashboard'))
        ...
        path_to_image = os.path.join(os.path.abspath(os.path.dirname(__file__)), 'static', f'{img_id}.jpg')
        if os.path.exists(path_to_image):
            os.remove(path_to_image)
        flash('Item deleted successfully along with its image')
        return redirect(url_for('viewitems'))
    return redirect(url_for('slogin'))
#===============================view bid items
...
def viewbiditems():
    if session.get('seller'):
        ...
        if itemid:
        #print("================================",itemid)
            cursor.execute('select * from items where item_id=%s',[itemid[0],])
            items=cursor.fetchall()
            #print("==============================",items)
            return render_template('adminviewauction.html',details=details,items=items)
        flash('no items are bidded')
        return redirect(url_for('seller_dashboard'))
    return redirect(url_for('slogin'))
...
def ordersdetails():
    if session.get('seller'):
        ...
        itemid=cursor.fetchone()
        if itemid:
            cursor.execute('select * from orders where item_id=%s',[itemid[0]])
            data=cursor.fetchall()
            cursor.execute('select * from items where item_id=%s',[itemid[0]])
            items=cursor.fetchall()
            return render_template('vieworders.html',orders=data,items=items)
        flash('no items are ordered')
        return redirect(url_for('seller_dashboard'))
    return redirect(url_for('slogin'))
